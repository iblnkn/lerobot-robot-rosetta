...
class ClassifierServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def _detect_image_size(self) -> tuple[int, int] | None:
        """
        Detect expected image size from the SpatialLearnedEmbeddings kernel.

        The kernel shape is (channel, height, width, num_features), where
        height/width are the expected feature-map spatial dims. ResNet-18
        downsamples by 32x, so the input resolution is (height * 32, width * 32).
        """
